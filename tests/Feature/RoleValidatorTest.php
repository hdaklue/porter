...
test('content-aware caching works correctly', function () {
...
class NewRole extends BaseRole {
    ...
    // Third call - should detect content change and invalidate cache
    $roles3 = RoleValidator::getExistingRoles($this->tempDir);
    
    // Content should be different now
    expect($roles3)->not->toBe($roles1);
});
...
test('handles complex level calculation scenarios', function () {
    // Create multiple roles with gaps in levels
    File::put($this->tempDir . '/Admin.php', '<?php
namespace App\Porter;
class Admin extends BaseRole {
    public function getLevel(): int { return 1; }
...
class Manager extends BaseRole {
    ...
    File::put($this->tempDir . '/Editor.php', '<?php
namespace App\Porter;
class Editor extends BaseRole {
    public function getLevel(): int { return 10; }
}');

    // Test 'lower' mode with Manager (level 5)
    [$level, $updates] = RoleValidator::calculateLevel('lower', 'Manager', $this->tempDir);
    
    expect($level)->toBe(5);
    expect($updates)->toHaveCount(2); // Manager and Editor should be updated
    
    // Verify Manager gets pushed to level 6
    $managerUpdate = collect($updates)->firstWhere('name', 'Manager');
    expect($managerUpdate['new_level'])->toBe(6);
    
    // Verify Editor gets pushed to level 11
    $editorUpdate = collect($updates)->firstWhere('name', 'Editor');
    expect($editorUpdate['new_level'])->toBe(11);
});
